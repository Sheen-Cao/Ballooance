...
# ── 路径 ────────────────────────────────────────────────────────────
MODEL_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "../models/ballooance.xml"))
...
def run_simulation():
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running():
            ...
            # 更新命令端 balloon_d（目标气量），写入 ctrl
            #   geom_size 不在这里改，等 mj_step 后从真实 qpos 同步
            for i in range(8):
                ...
                # HOLD: balloon_d 保持，ctrl 也不变

                data.ctrl[i] = balloon_d[i] - D_MIN

            # 检查命令文件（来自 send_command.py）
            if step % 100 == 0 and os.path.exists(CMD_FILE):
                try:
            ...
            # 关键修复：geom_size 跟随真实 qpos，被卡住时不会"鼓"出来
            for i in range(8):
